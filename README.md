...
1. What are the main differences between a stateful and a functional component?
  ...
  A.  Stateful Components (class components) accept, modify state/ dataa with implemented logic
      While Functional Components (stateless componets)  Serve as a way to modify what they user see determined by the data it receives.
        [most likely a class compoentn is changing the data on a functional component to display the user something different]

2. When does a componentWillMount function be called? What about a componentWillUpdate?

  A.  componentWillMount() is a call that happens before it render(). serving as a "heads up" and not causing a re-render() since render() knew about it.
      componentWillUpdate() gets called after a component has ben updated with shouldComponentUpdate() which itself is called before render() letting it know that this component will be updated this render().

3. Define stateful logic.

  A. It defines Data, it does not modify any Visual elements but simple gets something and returns something after doing some sort of cahnge to it, or storing data. point is that
  it is logic that does something rather than changes a display.
